...
existing_num_nodes() {
    local -r c=$1
    echo $c | jq -r '.parameters.kubernetes_worker_instances'
}

external_hostname() {
    local -r cc=$1

    if [[ "$(grep external_hostname "$cc")" == "" ]]; then
...
network_profile() {
    ...
    if [[ "$(grep network_profile "$cc")" == "" ]]; then
       ...
       # return NP name from config
       np=$(yqx '.network_profile' "$cc")
       echo "--network-profile $np"
    fi
}
...
kubernetes_profile() {
    ...
    if [[ "$(grep kubernetes_profile "$cc")" == "" ]]; then
       ...
       kp=$(yqx '.kubernetes_profile' "$cc")
       echo "--kubernetes-profile $kp"
    fi
}
...
compute_profile() {
    ...
    if [[ "$(grep compute_profile "$cc")" == "" ]]; then
       # Error out if KP is not defined in config
       echo "No Compute Profile defined in $cc...skipping"
    else
       # return CP name from config
       cp=$(yqx '.compute_profile' "$cc")
       echo "--compute-profile $cp"
    fi
}
...
check_kubernetes_profile_exists() {
    ...
    local -r kp_name=$(kubernetes_profile $cc)
...
converge() {
  ...
  for dir in ${cluster_dir}/*; do
      ...
      then
          ...
          check_kubernetes_profile_exists $cluster $cluster_dir
